...
// cobalt-scripts.js (157,3)
// https://cdn.polyfill.io/v2/polyfill.js

// START
/* Polyfill service v3.25.1
 * For detailed credits and licence information see https://github.com/financial-times/polyfill-service.
 * UA detected: chrome/77.0.0
...
// Initialize function
var bfi_counter = 0;
function bfi_init(element, options = null) {
	...
	form.each(function() {
		let bfi = this.querySelectorAll('input[type="file"]');
		...
		for (let i = 0; i < total; i++) {
			...
			if (input.hasAttribute('multiple')) {
				...
				label = '<span class="upload-icon"><img src="https://cdn2.hubspot.net/hubfs/2241343/ivoclar2019/ivoclar%20icons/file-upload.svg" alt="Upload icon"></span><span class="upload-text">Drop your files here</span>';
			} else {
		...
		Array.prototype.forEach.call(document.querySelectorAll('input[type="file"].bfi'), el => { el.classList.remove('bfi'); });
		if (options != null) {
			let style_override = '';
...
function bfi_reset() {
	...
	if (styles.length) {
		Array.prototype.forEach.call(styles, el => {
			el.remove();
		});
	}
}
...
window.addEventListener('dragover', e => {
	...
	if (dt.types && (dt.types.indexOf ? dt.types.indexOf('Files') != -1 : dt.types.contains('Files'))) {
		Array.prototype.forEach.call(document.querySelectorAll('.bfi-container'), el => {
		...
		bfi_drag_timeout = setTimeout(() => {
			Array.prototype.forEach.call(document.querySelectorAll('.bfi-container'), el => { el.classList.remove('expanded'); });
		}, 100);
		Array.prototype.forEach.call(document.querySelectorAll('.bfi-shadow'), el => {
...
document.addEventListener('change', e => {
	if (e.target.classList.contains('bfi-converted')) {
		...
		if (e.target.files.length) {
			container.classList.add('bfi-container-active');
			container.querySelector('.bfi-label').style.display = 'none';
			Array.prototype.forEach.call(container.querySelectorAll('.bfi-file'), el => { el.remove(); });
			...
			container.classList.remove('bfi-container-active');
